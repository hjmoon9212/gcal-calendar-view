...
| 시각(⏰) 주기 | 종일 줄 → 그리드 드래그 | 액션시트의 시각 선택기, 또는 **배치 모드** |
...
**액션시트** (행·칩·블록을 탭하면 뜬다) — `📅 마감일`(오늘 / 내일 / +7일 / 날짜 선택) · `🛫 시작일` · `↔ 기간째 이동`(🛫 를 옮기고 기간 길이 유지 — 데스크탑의 그냥 드래그와 같다) · `⏰ 시각` 지정/제거 · `🗓 시간대 골라 놓기` · `✏️ 편집` · `📄 원본 열기`.

**배치 모드** — 시각을 숫자로 고르는 것과 **그 날 무엇이 이미 차 있는지 보고 빈 곳에 놓는 것**은 다른 일이다. 액션시트의 `🗓 시간대 골라 놓기` 를 누르면 그 task 의 날로 일간이 열리고 안내 바가 뜬다. **빈 시간대를 탭하면 그 시각이 붙는다**(15분 스냅, 길이는 유지 — 데스크탑의 「종일 줄 → 그리드 드래그」와 같은 일). 배치 중에는 새벽에도 놓을 수 있게 0~24시를 전부 그린다.

> 배치 모드가 아닐 때 빈 시간대 탭은 **아무 일도 하지 않는다.** 스크롤하려다 스친 탭으로 시각이 붙으면 되돌릴 방법이 없다.
