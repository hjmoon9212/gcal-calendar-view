...
| | 데스크탑 | 모바일 |
|---|---|---|
| 화면 | 월간 / 주간 / 일간 기간 막대 | **월간 요약(색 점) + 날짜별 목록** |
| 날짜 바꾸기 | 막대 드래그 | 행을 **탭** → 액션시트 |
| 편집 모달 | 우클릭 | 액션시트의 `✏️ 편집` |
| 시각(⏰) | 일간 보기 드래그 | 액션시트의 시각 선택기 (15분 단위) |
| GCal 일정(📆) | 표시됨 | **표시 안 됨** (아래) |

액션시트에서 할 수 있는 것: `📅 마감일`(오늘 / 내일 / +7일 / 날짜 선택) · `🛫 시작일` · `↔ 기간째 이동`(🛫 를 옮기고 기간 길이 유지 — 데스크탑의 그냥 드래그와 같다) · `⏰ 시각` 지정/제거 · `✏️ 편집` · `📄 원본 열기`.

월간 요약의 날짜를 탭하면 **그 날에 걸친 항목만** 아래에 남는다. 다시 탭하거나 `선택 해제` 로 푼다.
