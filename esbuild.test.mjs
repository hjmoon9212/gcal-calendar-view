 ...
 *
 * tasks-gcal-sync 저장소의 것을 그대로 가져왔다(0.7.0) — 두 플러그인의 테스트 방식을 맞춘다.
 * obsidian 모듈은 tests/obsidian-stub.ts 로 alias 한다. main.js 가 obsidian 의 값(Plugin·Modal·
 * Setting·Platform…)을 쓰므로 스텁 없이는 번들 자체가 안 된다. 스텁은 실제 계약을 흉내내야 한다.
 */
